...
public class LegacyElasticsearchSpanStore implements AsyncSpanStore {
    ...
    public LegacyElasticsearchSpanStore(ElasticsearchStorage es) {
    ...
    @Override public void getTraces(final QueryRequest request, final Callback<List<List<Span>>> callback) {
        ...
        final List<String> indices = indexNameFormatter.formatTypeAndRange(null, beginMillis, endMillis);
        if (indices.isEmpty()) {
            callback.onSuccess(Collections.<List<Span>>emptyList());
            return;
        }
        ...
        final Callback<List<Span>> successCallback = new Callback<List<Span>>() {
    ...
    @Override public void getTrace(long traceIdHigh, long traceIdLow, final Callback<List<Span>> callback) {
