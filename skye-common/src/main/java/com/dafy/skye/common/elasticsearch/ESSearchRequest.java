 ...
 */
public final class ESSearchRequest {
    /**
    ...
    ESSearchRequest(List<String> indices, String type) {
    ...
    public ESSearchRequest filters(Filters filters) {
        return query(new BoolQuery("must", filters));
    }

    static ESSearchRequest.BoolQuery _nestedTermsEqual(Collection<String> nestedFields, String value) {
        List<NestedBoolQuery> conditions = new ArrayList<>();
        for (String nestedField : nestedFields) {
            conditions.add(new NestedBoolQuery(nestedField.substring(0, nestedField.indexOf('.')), "must",
                    new ESSearchRequest.Term(nestedField, value)));
        }
        return new ESSearchRequest.BoolQuery("should", conditions);
    }

    public static ESSearchRequest forIndicesAndType(List<String> indices, String type) {
        return new ESSearchRequest(indices, type);
    }

    public ESSearchRequest term(String field, String value) {
        return query(new Term(field, value));
    }

    public ESSearchRequest terms(String field, List<String> values) {
        return query(new Terms(field, values));
    }

    public ESSearchRequest addAggregation(Aggregation agg) {
    ...
    ESSearchRequest query(Object filter) {
        query = Collections.singletonMap("bool", Collections.singletonMap("filter", filter));
        return this;
    }
    public ESSearchRequest addSort(String field, SortOrder order){
        if(this.sort==null){
